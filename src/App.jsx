...
function App() {
  ...
  function initWorkoutBuilder(base) {
    if (base) {
      ...
      setWbDuration(split.hhmm);
      setWbDurSec(split.sec !== 0 && split.sec !== "" ? String(split.sec) : "");
  ...
  function initBuilderFromTemplate(tpl, customize = false) {
    if (customize) {
      ...
      setWizardTemplatePlan(tpl.custom ? null : {
        ...tpl,
        customize: true
      });
      setPlanView("builder");
  ...
  function handlePlanWizardSave(planData) {
    if (planData.isEdit) {
      ...
      setProfile(pr => ({
        ...pr,
        plans: pr.plans.map(pl => pl.id === planData.id ? {
          ...pl,
          ...rest
        } : pl)
      }));
  ...
  function savePlanEdits(plan) {
    setProfile(p => ({
      ...p,
      plans: p.plans.map(pl => pl.id === plan.id ? plan : pl)
    }));
    setActivePlan(plan);
    showToast("Plan saved! ✦");
  }
  function startPlanWorkout(plan) {
    ...
    plan.days.forEach(day => {
      day.exercises.forEach(ex => {
        const exData = allExById[ex.exId];
        if (!exData) return;
        const earned = calcExXP(ex.exId, ex.sets, ex.reps, profile.chosenClass, allExById, null, ex.weightLbs || null, null);
        totalXP += earned;
        entries.push({
          exercise: exData.name,
          icon: exData.icon,
          ...
          sourcePlanIcon: plan.icon,
          sourceGroupId: batchId,
          sourceTotalCal: day.totalCal || null,
          sourceActiveCal: day.activeCal || null,
    ...
    QUESTS.filter(q => q.auto && !_optionalChain([newQuests, 'access', _71 => _71[q.id], 'optionalAccess', _72 => _72.completed])).forEach(q => {
      if (checkQuestCompletion(q, newLog, profile.checkInStreak)) newQuests[q.id] = {
        completed: true,
        completedAt: todayStr(),
        claimed: false
    ...
    setPlanView("list");
    setActivePlan(null);
    showToast(`Plan complete! ${formatXP(totalXP, {
      signed: true
    })} claimed!` + ciSuffix);
  }
  const rootStyle = {
  ...
  if (mfaChallengeScreen) return <div style={{
    ...
    padding: "20px"
      ...
      alignItems: "center"
        ...
        /* Authenticator code input */{!mfaRecoveryMode && <div style={{
          display: "flex",
          flexDirection: "column",
          gap: S.s10
        }}><div style={{
  ...
  if (screen === "login") return <div style={{
    minHeight: "100vh",
    ...
    backgroundSize: "cover, cover, 200% auto",
        ...
        /* ── Auth Card ── */<div style={{
          ...
          gap: S.s0
            ...
            marginBottom: S.s18
              ...
              flex: 1,
              height: 1,
              background: "linear-gradient(90deg, transparent, rgba(180,172,158,.1))"
            }} /><span style={{
              fontSize: FS.fs50,
              color: "rgba(180,172,158,.35)",
              letterSpacing: ".16em",
              textTransform: "uppercase",
              whiteSpace: "nowrap"
            }}>{loginSubScreen === "forgot-pw" ? "\u2BF6 Reset Password \u2BF6" : loginSubScreen === "forgot-username" ? "\u2BF6 Find Your Account \u2BF6" : authIsNew ? "\u2BF6 Create Your Legacy \u2BF6" : "\u2BF6 Enter the Realm \u2BF6"}</span><div style={{
              flex: 1,
              height: 1,
              background: "linear-gradient(90deg, rgba(180,172,158,.1), transparent)"
            }} /></div>

          /* ── FORGOT PASSWORD SUB-SCREEN ── */{loginSubScreen === "forgot-pw" && <><div style={{
              fontSize: FS.lg,
              color: "#8a8478",
              lineHeight: 1.6,
              marginBottom: S.s14,
              textAlign: "center"
            }}>{"Enter the email address you used to create your account. We’ll send you a link to reset your password."}</div><div style={{
              marginBottom: S.s14
            }}><label style={{
                fontSize: FS.sm,
                color: "#8a8478",
                ...
                display: "block",
                marginBottom: S.s6
              }}>{"Email Address"}</label><input className={"inp"} type={"email"} value={forgotPwEmail} style={{
                fontSize: FS.fs88,
                width: "100%",
                padding: "10px 14px",
                ...
                setAuthMsg(null);
              }} placeholder={"you@example.com"} onKeyDown={e => {
                if (e.key === "Enter") sendPasswordReset();
              }} /></div>{authMsg && <div style={{
              fontSize: FS.fs80,
              color: authMsg.ok === true ? UI_COLORS.success : UI_COLORS.danger,
              textAlign: "center",
              padding: "4px 0 10px",
              lineHeight: 1.5
            }}>{authMsg.text}</div>}<button style={{
              width: "100%",
              padding: "12px",
              borderRadius: R.xl,
              border: "none",
              ...
              cursor: "pointer",
              transition: "all .2s"
            }} disabled={!forgotPwEmail.trim() || authLoading} onClick={sendPasswordReset}>{authLoading ? "Sending\u2026" : "\uD83D\uDCE7 Send Reset Link"}</button><div style={{
              borderTop: "1px solid rgba(45,42,36,.18)",
              marginTop: S.s14,
              paddingTop: 12,
              display: "flex",
              ...
              alignItems: "center"
                ...
                setForgotLookupResult(null);
                setForgotPrivateId("");
              }}>{"Forgot your email?"}</span></div></>

          /* ── FORGOT USERNAME SUB-SCREEN ── */}{loginSubScreen === "forgot-username" && <><div style={{
              fontSize: FS.lg,
              color: "#8a8478",
              lineHeight: 1.6,
              marginBottom: S.s14,
              textAlign: "center"
            }}>{"Enter your "}<strong style={{
                color: "#b4ac9e"
              }}>{"Private Account ID"}</strong>{" to look up the email on your account."}</div><div style={{
              marginBottom: S.s14
            }}><label style={{
                fontSize: FS.sm,
                color: "#8a8478",
                ...
                display: "block",
                marginBottom: S.s6
              }}>{"Private Account ID"}</label><input className={"inp"} type={"text"} value={forgotPrivateId} style={{
                fontSize: FS.fs88,
                width: "100%",
                padding: "10px 14px",
                background: "linear-gradient(145deg, rgba(32,30,26,.5), rgba(20,18,14,.35))",
                ...
                setForgotPrivateId(e.target.value);
                setForgotLookupResult(null);
              }} placeholder={"e.g. xP4mRk7bN2cQ"} onKeyDown={e => {
                if (e.key === "Enter") lookupByPrivateId();
              }} /></div>{forgotLookupResult && <div style={{
              ...
              marginBottom: S.s14,
              textAlign: "center"
            }}>{forgotLookupResult.found ? <><div style={{
                  fontSize: FS.fs68,
                  color: UI_COLORS.success,
                  marginBottom: S.s4
                }}>{"✓ Account found!"}</div><div style={{
                  fontSize: FS.fs82,
                  color: "#d4cec4",
                  fontWeight: 700,
                  letterSpacing: ".04em",
                  fontFamily: "monospace"
                }}>{forgotLookupResult.masked_email}</div><div style={{
                  fontSize: FS.fs58,
                  color: "#8a8478",
                  marginTop: S.s6
                }}>{"Use this email on the password reset screen."}</div><button style={{
                  marginTop: S.s8,
                  ...
                  setAuthMsg(null);
                  setForgotLookupResult(null);
                }}>{"→ Go to Password Reset"}</button></> : <div style={{
                fontSize: FS.lg,
                color: UI_COLORS.danger
              }}>{forgotLookupResult.error}</div>}</div>}{!forgotLookupResult?.found && <button style={{
              width: "100%",
              padding: "12px",
              borderRadius: R.xl,
              border: "none",
              ...
              cursor: "pointer",
              transition: "all .2s"
            }} disabled={!forgotPrivateId.trim() || authLoading} onClick={lookupByPrivateId}>{authLoading ? "Looking up\u2026" : "\uD83D\uDD0D Look Up My Email"}</button>}<div style={{
              ...
              textAlign: "center"
                ...
                lineHeight: 1.5
                  ...
                  fontWeight: 600
                }}>{"support@aurisargames.com"}</span>{" and we’ll help you recover your account."}</div></div><div style={{
              borderTop: "1px solid rgba(45,42,36,.18)",
              marginTop: S.s14,
              paddingTop: 12
            }}><span style={{
                ...
                setForgotLookupResult(null);
                setForgotPrivateId("");
              }}>{"← Back to Password Reset"}</span></div></>

          /* ── NORMAL LOGIN / SIGNUP FORM ── */}{loginSubScreen === null && <><div style={{
              marginBottom: S.s10
            }}><label style={{
                fontSize: FS.sm,
                color: "#8a8478",
                ...
                display: "block",
                marginBottom: S.s6
              }}>{"Email Address"}</label><input className={"inp"} type={"email"} value={authEmail} style={{
                fontSize: FS.fs88,
                width: "100%",
                padding: "10px 14px",
                background: "linear-gradient(145deg, rgba(32,30,26,.5), rgba(20,18,14,.35))",
            ...
            /* Password */<div style={{
              marginBottom: S.s14
                ...
                outline: "none",
                boxSizing: "border-box"
              }} onChange={e => setAuthPassword(e.target.value)} placeholder={"\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022"} onKeyDown={e => {
                if (e.key === "Enter") handleAuthSubmit();
              }} /></div>

            /* Remember me */<div style={{
              display: "flex",
              alignItems: "center",
              gap: S.s8,
              cursor: "pointer",
              marginBottom: S.s16
            }} onClick={() => setAuthRemember(r => !r)}><div style={{
                width: 17,
                height: 17,
                borderRadius: R.r4,
                flexShrink: 0,
            ...
            /* Feedback message */{authMsg && <div style={{
              fontSize: FS.fs80,
              color: authMsg.ok === true ? UI_COLORS.success : UI_COLORS.danger,
              textAlign: "center",
              padding: "4px 0 10px",
            ...
            }} disabled={authLoading || !authEmail.trim() || !authPassword.trim()} onClick={handleAuthSubmit}>{authLoading ? authIsNew ? "Forging your legacy\u2026" : "Entering the realm\u2026" : authIsNew ? "\u2694\uFE0F Create Account" : "\u2694\uFE0F Enter the Realm"}</button>
            ...
            /* Footer links */<div style={{
              ...
              alignItems: "center"
                ...
                setAuthMsg(null);
                setAuthPassword("");
              }}>{"Create a Profile"}</span> : <span style={{
                fontSize: FS.lg,
                color: "#8a8478",
                cursor: "pointer"
              }} onClick={() => {
                ...
                setAuthMsg(null);
                setForgotPwEmail("");
              }}>{"Forgot password?"}</span>}</div></> /* end loginSubScreen===null */

          /* Preview mode — PIN-gated dev access (stripped from production builds via PREVIEW_ENABLED) */}{PREVIEW_ENABLED && <div style={{
            ...
            textAlign: "center"
              ...
              if (!previewPinEnabled) {
                ...
                marginBottom: S.s6
                  ...
                  if (e.key === "Enter") {
                    if (previewPinInput === PREVIEW_PIN) {
                      launchPreviewMode();
                    } else {
                      setPreviewPinError(true);
                    }
                  }
                }} autoFocus={true} /><button className={"preview-pin-go"} onClick={() => {
                  if (previewPinInput === PREVIEW_PIN) {
                    launchPreviewMode();
                  } else {
                    setPreviewPinError(true);
                  }
                }}>{"Go"}</button></div>{previewPinError && <div style={{
                fontSize: FS.fs55,
                color: UI_COLORS.danger,
                marginTop: S.s4
  ...
  return <div className={"root"} style={rootStyle}><style>{CSS}</style><div className={"bg"} />{PARTICLES.map(p => <div key={p.id} className={"pt"} style={{
    ...
    /* ══ MAIN ═══════════════════════════════════ */}{screen === "main" && clsKey && <div className={"hud"} style={activeTab === "messages" && msgView === "chat" ? {
      ...
      /* ══ DROPDOWN MENU — rendered outside hud-top to escape backdrop-filter stacking context ══ */{navMenuOpen && <div onClick={() => setNavMenuOpen(false)} style={{
        position: "fixed",
        inset: 0,
        zIndex: 900
      ...
      /* ══ BOTTOM TAB BAR — fixed iOS material ══ */}<div className={"hud-nav-panel"}><div className={"tabs"}>{[["workout", "Exercises", "mdi:dumbbell"], ["workouts", "Workouts", "mdi:weight-lifter"], ["calendar", "Calendar", "mdi:calendar-blank"], ["character", "Character", "game-icons:crossed-swords"], ["social", "Guild", "game-icons:tribal-pendant"]].map(([t, l, iconName]) => {
        ...
        paddingBottom: 0
          ...
          /* ══ EXERCISES SUB-TAB BAR ══ */<div className={"log-subtab-bar"} style={{
            marginBottom: S.s14
          ...
          /* ══ LOG SUB-TAB (original grimoire view) ══ */{exSubTab === "log" && <><div className={"techniques-header"}><div className={"tech-hdr-left"}><div className={"tech-ornament-line tech-ornament-line-l"} /><span className={"tech-hdr-title"}>{"✦ Techniques ✦"}</span><div className={"tech-ornament-line tech-ornament-line-r"} /></div></div>

            /* ══ TECHNIQUE SEARCH ══ */<div className={"tech-search-wrap"}><span className={"tech-search-icon"}>{"🔍"}</span><input className={"tech-search-inp"} placeholder={"Search Techniques…"} value={exSearch} onChange={e => setExSearch(e.target.value)} />{exSearch && <span className={"tech-search-clear"} onClick={() => setExSearch("")}>{"✕"}</span>}</div>

            /* ══ FILTERS ══ */<div className={"filter-section"}><div className={"filter-pills-row"}>{[{
                  cat: "strength",
                  icon: "⚔",
                  label: "Strength"
                }, {
            ...
            /* ══ COMMAND ACTION BAR ══ */{multiMode && multiSelEx.size > 0 && <div className={"command-action-bar"}><div className={"cab-count"}><span className={"cab-rune"}>{"⊞"}</span><span className={"cab-num"}>{multiSelEx.size}</span></div><div className={"cab-actions"}><button className={"cab-btn"} onClick={() => {
          ...
          /* ══ LIBRARY SUB-TAB ══ */}{exSubTab === "library" && (() => {
            ...
            return <div> /* Sticky search bar — translucent material */
              ...
              libBrowseMode === "home" && <div>{/* Your Exercises — hero carousel */
                yourExercises.length > 0 && <div className={"lib-home-section"} style={{
                  marginBottom: S.s4
                      ...
                      })}</div></div></div>}{yourExercises.length > 0 && <div className={"lib-divider"} />} /* Browse by Muscle — feature tiles */
              ...
              libBrowseMode === "filtered" && <div> /* Back to browse */
                <div style={{
                  marginBottom: S.s10
                    ...
                    alignItems: "center",
                    gap: S.s4
                  }}>{"← Browse Library"}</button></div> /* Filter dropdowns row — custom panels that stay open for multi-select */
                <div style={{
                  display: "flex",
                  gap: S.s8,
                  marginBottom: S.s10,
                ...
                (libTypeFilters.size > 0 || libMuscleFilters.size > 0 || libEquipFilters.size > 0) && <div style={{
                  ...
                  marginBottom: S.s8
                    ...
                    alignItems: "center",
                    gap: S.s4
                  }}>{v.charAt(0).toUpperCase() + v.slice(1)}{" ✕"}</span>)}</div>} /* Count + clear row */
                <div style={{
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "space-between",
                ...
                libSelectMode && libSelected.size > 0 && <div style={{
                  ...
                  gap: S.s8
                    ...
                    justifyContent: "center"
                      ...
                      whiteSpace: "nowrap",
                      textAlign: "center"
                    }}>{"📋 Plan"}</button></div></div>} /* Exercise list (paginated) */
                <div style={{
                  display: "flex",
                  flexDirection: "column",
                  gap: S.s6
                    ...
                    return <div key={ex.id} className={`picker-ex-row${isSel ? " sel" : ""}`} onClick={() => {
                      ...
                      <div className={"picker-ex-orb"}><ExIcon ex={ex} size={"1rem"} color={"#d4cec4"} /></div> /* Body */
                      <div style={{
                        flex: 1,
                        minWidth: 0
                      }}><div style={{
                ...
                justifyContent: "center"
                  ...
                  padding: "20px 18px 32px"
                    ...
                    marginTop: S.s8
                      ...
                      cursor: "pointer",
                      textAlign: "center"
                    }}>{"📋 Add to Plan"}</button></div> /* Edit & Complete Now */
                  <button onClick={() => {
          ...
          /* ══ MY WORKOUTS SUB-TAB ══ */}{exSubTab === "myworkouts" && <div><div style={{
              ...
              /* Multi-select action bar */{favSelectMode && favSelected.size > 0 && <div style={{
                background: "rgba(45,42,36,.2)",
                border: "1px solid rgba(180,172,158,.06)",
                borderRadius: R.r10,
                padding: "10px 14px",
        ...
        /* ── WORKOUTS TAB ────────────────────── */}{activeTab === "workouts" && (() => {
          ...
          if (workoutView === "list") return <><div className={"wo-sticky-filters"}><div style={{
              ...
              /* Subtabs */<div className={"log-subtab-bar"} style={{
                marginBottom: S.s0
            ...
            /* Label filter dropdown */{(profile.workoutLabels || []).length > 0 && <div style={{
              display: "flex",
              gap: S.s8,
              marginBottom: S.s10,
              position: "relative"
          ...
          if (workoutView === "recipes") {
            ...
            return <><div className={"wo-sticky-filters"}><div style={{
                ...
                /* Category multi-select dropdown */<div style={{
                  display: "flex",
                  gap: S.s8,
                  marginBottom: S.s0,
                  position: "relative"
                ...
                return <div key={tpl.id} className={"workout-card"} style={{
                  ...
                  /* Collapsible Description */{tpl.desc && <div style={{
                    position: "relative",
          ...
          if (workoutView === "builder") return <><div className={"builder-nav-hdr"}><button className={"btn btn-ghost btn-sm"} onClick={() => {
            ...
            /* Identity panel: Name + Icon + Description */<div className={"wb-section"}><div className={"wb-section-hdr"}><span className={"wb-section-hdr-icon"}>{"✦"}</span>{"Identity"}</div><div className={"field"}><label>{"Workout Name"}</label><input className={"inp"} value={wbName} onChange={e => setWbName(e.target.value)} placeholder={"e.g. Morning Push Day…"} /></div><div className={"field"}><label>{"Icon"}</label><div className={"icon-row"} style={{
                  flexWrap: "wrap",
                  gap: S.s6
                }}>{["💪", "🏋️", "🔥", "⚔️", "🏃", "🚴", "🧘", "⚡", "🎯", "🛡️", "🏆", "🌟", "💥", "🗡️", "🥊", "🤸", "🏊", "🎽", "🦵", "🦾"].map(ic => <div key={ic} className={`icon-opt ${wbIcon === ic ? "sel" : ""}`} style={{
                    fontSize: "1.2rem",
                    width: 36,
                    height: 36
            ...
            /* Labels panel */<div className={"wb-section"}><div className={"wb-section-hdr"}><span className={"wb-section-hdr-icon"}>{"❖"}</span>{"Labels"}<span style={{
                  ...
                  marginLeft: S.s6,
                  textTransform: "none"
                }}>{"(optional)"}</span></div><div style={{
                display: "flex",
                gap: S.s6,
                flexWrap: "wrap",
                alignItems: "center"
              }}>{(profile.workoutLabels || []).map(l => <span key={l} className={"wo-label-chip" + (wbLabels.includes(l) ? " sel" : "")} onClick={() => setWbLabels(prev => prev.includes(l) ? prev.filter(x => x !== l) : [...prev, l])}>{l}</span>)}<span style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: S.s4
                }}><input className={"wo-label-new-inp"} value={newLabelInput} onChange={e => setNewLabelInput(e.target.value)} onKeyDown={e => {
                    if (e.key === "Enter" && newLabelInput.trim()) {
                      const lbl = newLabelInput.trim();
                      if (!(profile.workoutLabels || []).some(x => x.toLowerCase() === lbl.toLowerCase())) {
                        setProfile(p => ({
            ...
            /* Stats panel: Duration / Calories */<div className={"wb-section"}><div className={"wb-section-hdr"}><span className={"wb-section-hdr-icon"}>{"⏱"}</span>{"Session Stats"}<span style={{
                  ...
                  marginLeft: S.s6,
                  textTransform: "none"
                }}>{"(optional)"}</span></div><div className={"wb-stats-row"}><div className={"field"} style={{
                  flex: 1.5,
                  marginBottom: S.s0
                  ...
                  flex: 0.8,
                  marginBottom: S.s0
                }}><label>{"Seconds"}</label><input className={"inp"} type={"number"} min={"0"} max={"59"} value={wbDurSec} onChange={e => setWbDurSec(e.target.value)} placeholder={"0"} /></div><div className={"field"} style={{
                  flex: 1,
                  marginBottom: S.s0
                }}><label>{"Active Cal"}</label><input className={"inp"} type={"number"} min={"0"} max={"9999"} value={wbActiveCal} onChange={e => setWbActiveCal(e.target.value)} placeholder={"e.g. 320"} /></div><div className={"field"} style={{
                  flex: 1,
                  marginBottom: S.s0
                }}><label>{"Total Cal"}</label><input className={"inp"} type={"number"} min={"0"} max={"9999"} value={wbTotalCal} onChange={e => setWbTotalCal(e.target.value)} placeholder={"e.g. 450"} /></div></div></div>
            /* Exercise list */<div className={"wo-section-hdr"} style={{
              marginTop: S.s18,
              marginBottom: S.s10
        ...
        /* ── PLANS TAB ───────────────────────── */}{activeTab === "plans" && <>{planView === "list" && <><div style={{
              display: "flex",
              alignItems: "center",
              marginBottom: S.s8
            }}><div className={"rpg-sec-header rpg-sec-header-center"}><div className={"rpg-sec-line rpg-sec-line-l"} /><span className={"rpg-sec-title"}>{"✦ Plans ✦"}<span className={"info-icon"} style={{
                    display: "inline-flex",
                    alignItems: "center",
                    justifyContent: "center",
                    width: 16,
                    ...
                    cursor: "pointer",
                    position: "relative"
                  }}>{"?"}<span className={"info-tooltip"}>{"For long term workout plans. May include individual exercises and/or pre-defined workouts."}</span></span></span><div className={"rpg-sec-line rpg-sec-line-r"} /></div></div><div style={{
              display: "flex",
              gap: S.s8,
              marginBottom: S.s14,
              flexWrap: "wrap"
            }}><button className={"btn btn-gold btn-sm"} onClick={initBuilderScratch}>{"＋ New Plan"}</button><button className={"btn btn-ghost btn-sm"} onClick={() => setPlanView("recipe-pick")}>{"📋 Recipes"}</button></div>

            /* ── UPCOMING section — standalone exercises only ─ */{(() => {
              const swAll = profile.scheduledWorkouts || [];
              if (swAll.length === 0) return null;
              const allItems = swAll.filter(s => !s.sourceWorkoutId) // exclude workout-grouped items — they show in Workouts tab
              .map(s => {
              ...
              return <div className={"upcoming-section"}><div className={"sec"} style={{
                  marginBottom: S.s8
                }}>{"📅 Scheduled Exercises"}</div>{allItems.map(item => {
                  ...
                  return <div key={item.id} className={"upcoming-card"}><div className={"upcoming-icon"}>{item.icon}</div><div className={"upcoming-info"}><div className={"upcoming-name"}>{item.name}</div><div className={"upcoming-date"}>{formatScheduledDate(item.date)}{item.notes ? <span style={{
                      ...
                      e.stopPropagation();
                      openScheduleEx(item.exId || item.id, item.id);
                    }}>{"✎"}</div><div className={"upcoming-del"} onClick={e => {
                      e.stopPropagation();
                      removeScheduledWorkout(item.id);
                    }}>{"✕"}</div></div>;
                })}<div className={"div"} style={{
                  margin: "6px 0"
                }} /></div>;
            })()}{profile.plans.length === 0 && <div className={"empty"}>{"No plans yet."}<br />{"Create one or browse recipes."}</div>}{profile.plans.map(plan => {
              ...
              return <div key={plan.id} className={"plan-card"} style={{
                "--pc": cls && cls.color || "#b4ac9e"
              }}><div className={"plan-card-top"} onClick={() => {
                  initBuilderFromTemplate(plan, true);
                }}><div className={"plan-icon"}>{plan.icon}</div><div style={{
                    flex: 1,
                    minWidth: 0
                  }}><div style={{
                      display: "flex",
                      ...
                      marginBottom: S.s2
                        ...
                        flex: 1
                      }}>{plan.name}</div>{plan.level && <span className={`plan-level-badge ${plan.level.toLowerCase()}`} style={{
                        flexShrink: 0
                      }}>{plan.level}</span>}</div><div className={"plan-meta"}><span className={`plan-type-badge type-${plan.type}`}>{plan.durCount && plan.durCount > 1 ? `${plan.durCount} ${plan.type}s` : plan.type}</span><span style={{
                        marginLeft: S.s6,
                        fontSize: FS.sm,
                        color: "#8a8478"
                      }}>{plan.days.filter(d => d.exercises.length > 0).length}{" active days"}</span>{plan.startDate && <span style={{
                        marginLeft: S.s6,
                        fontSize: FS.sm,
                        color: "#8a8478"
                      }}>{"📅 "}{new Date(plan.startDate + "T12:00:00").toLocaleDateString([], {
                          month: "short",
                          day: "numeric",
                          year: "numeric"
                        })}{plan.endDate ? " → " + new Date(plan.endDate + "T12:00:00").toLocaleDateString([], {
                          month: "short",
                          day: "numeric",
                          year: "numeric"
                        }) : ""}</span>}{!plan.startDate && hasSched && <span style={{
                        marginLeft: S.s6,
                        fontSize: FS.sm,
                        color: "#b4ac9e"
                      }}>{"📅 "}{formatScheduledDate(plan.scheduledDate)}</span>}</div></div><div className={"plan-xp-badge"}>{"⚡ "}{planXP.toLocaleString()}</div></div>{plan.description && <div className={"plan-desc"} onClick={() => {
                  initBuilderFromTemplate(plan, true);
                }}>{plan.description}</div>}<div style={{
                  display: "flex",
                  gap: S.s8,
                  marginTop: S.s8,
                  paddingTop: 7,
                  borderTop: "1px solid rgba(45,42,36,.18)"
                }}><button className={`plan-sched-btn ${hasSched ? "plan-sched-active" : ""}`} onClick={e => {
                    e.stopPropagation();
                    openSchedulePlan(plan);
                  }}>{hasSched ? "📅 " + formatScheduledDate(plan.scheduledDate) : "📅 Schedule"}</button><div style={{
                    flex: 1
                  }} /><button className={"btn btn-ghost btn-xs"} onClick={e => {
                    e.stopPropagation();
                    initBuilderFromTemplate(plan, true);
                  }}>{"View →"}</button></div></div>;
            })}</>}{planView === "recipe-pick" && <><div style={{
              display: "flex",
              alignItems: "center",
              gap: S.s8,
              marginBottom: S.s12
            }}><button className={"btn btn-ghost btn-sm"} onClick={() => setPlanView("list")}>{"← Back"}</button><div className={"sec"} style={{
                margin: 0,
                border: "none",
                padding: S.s0
              }}>{"Plan Recipes"}</div></div>{["day", "week"].map(type => {
              const typePlans = PLAN_TEMPLATES.filter(t => t.type === type);
              if (!typePlans.length) return null;
              return <div key={type}><div className={"sec"} style={{
                  textTransform: "capitalize",
                  marginBottom: S.s8
                }}>{type}{" Plans"}</div>{typePlans.map(tpl => {
                  ...
                  return <div key={tpl.id} className={"workout-card"} style={{
                    marginBottom: S.s10
                      ...
                      gap: S.s10,
                      cursor: "pointer"
                    }} onClick={() => setCollapsedTpls(s => ({
                      ...s,
                      [tpl.id]: !s[tpl.id]
                    }))}><div className={"workout-icon"} style={{
                        flexShrink: 0
                      }}>{tpl.icon}</div><div style={{
                        flex: 1,
                        minWidth: 0
                      }}><div style={{
                          display: "flex",
                          ...
                          gap: S.s6,
                          marginBottom: S.s4
                        }}><div className={"workout-name"} style={{
                            flex: 1
                          }}>{tpl.name}</div>{tpl.level && <span className={`plan-level-badge ${tpl.level.toLowerCase()}`} style={{
                            flexShrink: 0
                          }}>{tpl.level}</span>}</div><div className={"workout-meta"}><span className={"workout-tag"}>{activeDays.length}{" active day"}{activeDays.length !== 1 ? "s" : ""}</span><span className={"workout-tag"}>{formatXP(tplXP, {
                              prefix: "⚡ "
                            })}</span><span className={`plan-type-badge type-${tpl.type}`} style={{
                            marginLeft: S.s4
                          }}>{tpl.durCount && tpl.durCount > 1 ? `${tpl.durCount} ${tpl.type}s` : tpl.type}</span>{isRec && <span style={{
                            fontSize: FS.fs56,
                            color: _optionalChain([cls, 'optionalAccess', _95 => _95.color]),
                            marginLeft: S.s4
                          }}>{"✦ "}{_optionalChain([cls, 'optionalAccess', _96 => _96.name])}</span>}</div></div><span style={{
                        flexShrink: 0,
                        paddingTop: 2,
                        lineHeight: 1,
                        display: "flex",
                        ...
                        }}><defs><linearGradient id={"chevGrad"} x1={"0"} y1={"0"} x2={"0"} y2={"1"}><stop offset={"0%"} stopColor={"#b4ac9e"} /><stop offset={"100%"} stopColor={"#7a4e1a"} /></linearGradient></defs><polyline points={"4,7 9,12 14,7"} stroke={"url(#chevGrad)"} strokeWidth={"2"} strokeLinecap={"round"} strokeLinejoin={"round"} /></svg></span></div>
                    /* Expanded content */{!isCollapsed && (() => {
                      const allTplExIds = [...new Set(tpl.days.flatMap(d => d.exercises.map(e => e.exId)))];
                      return <div style={{
                        marginTop: S.s10
                      }}><div className={"workout-ex-pill-row"} style={{
                          marginBottom: S.s10
                        }}>{allTplExIds.slice(0, 6).map((exId, i) => {
                            const exD = allExById[exId];
                            return exD ? <span key={i} className={"workout-ex-pill"}>{exD.icon}{" "}{exD.name}</span> : null;
                          })}{allTplExIds.length > 6 && <span className={"workout-ex-pill"}>{"+"}{allTplExIds.length - 6}{" more"}</span>}</div>{tpl.description && <div style={{
                          fontSize: FS.lg,
                          color: "#8a8478",
                          fontStyle: "italic",
                          marginBottom: S.s12,
                          lineHeight: 1.6
                        }}>{tpl.description}</div>}<div style={{
                          background: "rgba(45,42,36,.12)",
                          border: "1px solid rgba(45,42,36,.18)",
                          borderRadius: R.lg,
                          padding: "8px 12px",
                          marginBottom: S.s12
                        }}>{tpl.days.map((day, di) => <div key={di} style={{
                            display: "flex",
                            alignItems: "flex-start",
                            gap: S.s8,
                            padding: "4px 0",
                            borderBottom: di < tpl.days.length - 1 ? "1px solid rgba(45,42,36,.15)" : ""
                          }}><span style={{
                              fontSize: FS.fs65,
                              color: "#b4ac9e",
                              minWidth: 50,
                              flexShrink: 0,
                              paddingTop: 1
                            }}>{day.label || `Day ${di + 1}`}</span><span style={{
                              fontSize: FS.fs68,
                              color: "#8a8478",
                              flex: 1
                                ...
                                return exD ? <span key={ei}>{ei > 0 ? " · " : ""}{exD.icon}{" "}{exD.name}{" "}{e.sets}{"×"}{e.reps}</span> : null;
                              })}</span></div>)}</div><div style={{
                          display: "flex",
                          gap: S.s8
                        }}><button className={"btn btn-ghost btn-sm"} style={{
                            ...
                            e.stopPropagation();
                            initBuilderFromTemplate(tpl, false);
                          }}>{"👁 View This Plan"}</button><button className={"btn btn-gold btn-sm"} style={{
                            flex: 1
                          }} onClick={e => {
                            e.stopPropagation();
                            initBuilderFromTemplate(tpl, true);
                          }}>{"✎ Customize First"}</button></div></div>;
                    })()}</div>;
                })}</div>;
            ...
            function updateDetailEx(dayI, exI, field, val) {
              const newDays = plan.days.map((d, di) => di !== dayI ? d : {
                ...d,
                exercises: d.exercises.map((e, ei) => ei !== exI ? e : {
                  ...e,
                  [field]: val
                })
              });
            ...
            return <><div style={{
                ...
                marginBottom: S.s14
                  ...
                  flex: 1
                    ...
                    fontSize: FS.fs86,
                    color: "#d4cec4"
                  }}>{plan.icon}{" "}{plan.name}{plan.level && <span className={`plan-level-badge ${plan.level.toLowerCase()}`} style={{
                      marginLeft: S.s8,
                      verticalAlign: "middle"
                    }}>{plan.level}</span>}</div></div><button className={"btn btn-ghost btn-sm"} style={{
                  flexShrink: 0
                }} onClick={() => initBuilderFromTemplate(plan, true)}>{"✎ Customize"}</button>{plan.custom && <button className={"btn btn-gold btn-sm"} onClick={() => savePlanEdits(plan)}>{"💾 Save"}</button>}</div><div className={"xp-projection"} style={{
                marginBottom: S.s12
              }}><div><div className={"xp-proj-label"}>{"Total Projected XP"}</div><div className={"xp-proj-detail"}>{plan.days.filter(d => d.exercises.length > 0).length}{" active days · "}{cls && cls.name}{" bonuses applied"}{plan.durCount && <span style={{
                      marginLeft: S.s6
                    }}>{"· "}<span className={`plan-type-badge type-${plan.type}`} style={{
                        verticalAlign: "middle"
                    ...
                    marginTop: S.s4
                        ...
                        day: "numeric",
                        year: "numeric"
                      })}</span>}{plan.startDate && plan.endDate && <span style={{
                      margin: "0 4px"
                    }}>{"→"}</span>}{plan.endDate && <span>{new Date(plan.endDate + "T12:00:00").toLocaleDateString([], {
                        month: "short",
                        day: "numeric",
                        year: "numeric"
                      })}</span>}{plan.startDate && plan.endDate && (() => {
                ...
                fontStyle: "italic",
                marginBottom: S.s8
              }}>{"Tip: Customize this plan to save weight/duration edits permanently."}</div>}{currentDay.exercises.length === 0 ? <div className={"empty"} style={{
                padding: "22px 0"
              }}>{"Rest day. Recover well."}</div> : currentDay.exercises.map((ex, exI) => {
                const exData = allExById[ex.exId];
                ...
                const clsD = profile.chosenClass ? CLASSES[profile.chosenClass] : null;
                const mult = clsD && clsD.bonuses && exData.category ? clsD.bonuses[exData.category] || 1 : 1;
                ...
                const runPace = isRunningEx && distMiVal > 0 && durationMin > 0 ? durationMin / distMiVal : null;
                const runBoostPct = runPace ? runPace <= 8 ? 20 : 5 : 0;
                return <div key={exI} className={`plan-ex-row ${dragDetailExIdx === exI ? "dragging" : ""}`} style={{
                  flexDirection: "column",
                  alignItems: "stretch",
                  gap: S.s0,
                  opacity: dragDetailExIdx === exI ? 0.5 : 1
                }} draggable={true} onDragStart={e => {
                  e.dataTransfer.effectAllowed = "move";
                  setDragDetailExIdx(exI);
                }} onDragOver={e => {
                  e.preventDefault();
                  e.dataTransfer.dropEffect = "move";
                }} onDrop={e => {
                  ...
                  setDragDetailExIdx(null);
                    ...
                    return <><div style={{
                        ...
                        marginBottom: collapsed ? 0 : 7
                          ...
                          gap: S.s2,
                          flexShrink: 0
                        }} /><span className={"plan-ex-icon"}>{exData.icon}</span><div style={{
                          flex: 1,
                          minWidth: 0
                        }}><div className={"plan-ex-name"}>{exData.name}</div>{exData.id !== "rest_day" && <div className={"plan-ex-sets"}>{noSetsEx ? "" : ex.sets + "×"}{ex.reps}{ex.weightLbs && <span style={{
                              color: "#8a8478",
                              marginLeft: S.s6
                            }}>{metric ? lbsToKg(ex.weightLbs) + " kg" : ex.weightLbs + " lbs"}</span>}{ex.durationMin && <span style={{
                              color: "#8a8478",
                              marginLeft: S.s6
                            }}>{ex.durationMin}{" min"}</span>}{ex.distanceMi && <span style={{
                              color: "#8a8478",
                              marginLeft: S.s6
                            }}>{metric ? parseFloat(miToKm(ex.distanceMi)).toFixed(1) + " km" : ex.distanceMi + " mi"}</span>}{ex.hrZone && <span style={{
                              color: HR_ZONES[ex.hrZone - 1].color,
                              marginLeft: S.s6
                            }}>{"Z"}{ex.hrZone}</span>}<span className={`ex-mult ${mult > 1.02 ? "mb" : mult < 0.98 ? "mp" : "mn"}`} style={{
                              marginLeft: S.s6
                            }}>{Math.round(mult * 100)}{"%"}</span></div>}</div>{(isRunningEx && pbDisp || exPBDisp2) && <span style={{
                          fontSize: FS.fs58,
                          color: "#b4ac9e",
                          flexShrink: 0
                        }}>{"🏆 "}{isRunningEx && pbDisp ? pbDisp : exPBDisp2}</span>}<div className={"plan-ex-xp"}>{"+"}{exXP}{" XP"}{runBoostPct > 0 && <span style={{
                            color: UI_COLORS.warning,
                            marginLeft: S.s2
                          }}>{"⚡"}</span>}</div><div className={"ex-info-btn"} style={{
                          position: "static"
                        }} onClick={() => {
                          setDetailEx(exData);
                          setDetailImgIdx(0);
                        }}>{"ℹ"}</div><span className={"ex-collapse-btn"} onClick={e => {
                          e.stopPropagation();
                          toggleDetailEx(vDayIdx, exI);
                        }}><svg width={"14"} height={"14"} viewBox={"0 0 14 14"} fill={"none"} xmlns={"http://www.w3.org/2000/svg"} style={{
                            transition: "transform .22s ease",
                            transform: collapsed ? "rotate(0deg)" : "rotate(180deg)"
                          }}><defs><linearGradient id={"cg2"} x1={"0"} y1={"0"} x2={"0"} y2={"1"}><stop offset={"0%"} stopColor={"#b4ac9e"} /><stop offset={"100%"} stopColor={"#7a4e1a"} /></linearGradient></defs><polyline points={"3,5 7,9 11,5"} stroke={"url(#cg2)"} strokeWidth={"1.8"} strokeLinecap={"round"} strokeLinejoin={"round"} /></svg></span></div>{!collapsed && exData.id !== "rest_day" && <><div style={{
                          display: "flex",
                          gap: S.s8,
                          marginBottom: S.s6
                        }}>{!noSetsEx && <div style={{
                            flex: 1
                              ...
                              marginBottom: S.s4,
                              display: "block"
                            }}>{"Sets"}</label><input className={"plan-ex-edit-inp"} style={{
                              width: "100%"
                            }} type={"text"} inputMode={"decimal"} value={ex.sets === 0 || ex.sets === "" ? "" : ex.sets || ""} onChange={e => updateDetailEx(vDayIdx, exI, "sets", e.target.value)} /></div>}<div style={{
                            flex: 1
                          }}><label style={{
                              fontSize: FS.sm,
                              color: "#b0a898",
                              marginBottom: S.s4,
                              display: "block"
                            }}>{isCardioEx ? "Duration (min)" : "Reps"}</label><input className={"plan-ex-edit-inp"} style={{
                              width: "100%"
                            }} type={"text"} inputMode={"decimal"} value={ex.reps === 0 || ex.reps === "" ? "" : ex.reps || ""} onChange={e => updateDetailEx(vDayIdx, exI, "reps", e.target.value)} /></div>{hasWeightEx && <div style={{
                            flex: 1
                          }}><label style={{
                              fontSize: FS.sm,
                              color: "#b0a898",
                              marginBottom: S.s4,
                              display: "block"
                            }}>{wUnit}</label><input className={"plan-ex-edit-inp"} style={{
                              width: "100%"
                            }} type={"text"} inputMode={"decimal"} step={metric ? "0.5" : "2.5"} value={inputWVal} placeholder={"—"} onChange={e => {
                              const v = e.target.value;
                              const lbs = v && metric ? kgToLbs(v) : v;
                          ...
                          color: UI_COLORS.warning,
                          marginBottom: S.s6
                        }}>{"⚡ +"}{runBoostPct}{"% pace bonus"}{runBoostPct === 20 ? " (sub-8 mi!)" : ""}</div>}{hasWeightEx && <div style={{
                          marginBottom: S.s6
                        }}><div style={{
                            display: "flex",
                            justifyContent: "space-between",
                            ...
                            marginBottom: S.s2
                              ...
                              color: "#b0a898",
                              marginBottom: S.s0
                            }}>{"Weight Intensity"}</label><span style={{
                              fontSize: FS.fs68,
                              color: "#b4ac9e"
                            }}>{ex.weightPct || 100}{"%"}</span></div><input type={"range"} className={"pct-slider"} min={"0"} max={"100"} step={"5"} value={pctToSlider(ex.weightPct || 100)} onChange={e => updateDetailEx(vDayIdx, exI, "weightPct", sliderToPct(Number(e.target.value)))} /><div style={{
                            display: "flex",
                            justifyContent: "space-between",
                            fontSize: FS.fs55,
                            color: "#8a8478",
                          ...
                          marginBottom: S.s6
                            ...
                            marginBottom: S.s4,
                            display: "block"
                          }}>{"Distance ("}{metric ? "km" : "mi"}{") "}<span style={{
                              opacity: .6,
                              fontSize: FS.fs55
                            }}>{"(optional)"}</span></label><input className={"plan-ex-edit-inp"} style={{
                            width: "100%"
                          }} type={"text"} inputMode={"decimal"} value={inputDistVal} placeholder={"0"} onChange={e => {
                            const v = e.target.value;
                            const mi = v && metric ? kmToMi(v) : v;
                            updateDetailEx(vDayIdx, exI, "distanceMi", mi || null);
                          }} /></div>}{isCardioEx && <div><label style={{
                            fontSize: FS.sm,
                            color: "#b0a898",
                            marginBottom: S.s4,
                            display: "block"
                ...
                if (!isUserPlan) {
                  return <div style={{
                    ...
                    textAlign: "center"
                      ...
                      color: "#8a8478",
                      marginBottom: S.s8
                    }}>{"This is a recipe preview. Customize it to add it to your plans."}</div><button className={"btn btn-gold"} style={{
                      width: "100%"
                    }} onClick={() => initBuilderFromTemplate(plan, true)}>{"✎ Customize & Add to My Plans"}</button></div>;
                }
                return <><div className={"plan-actions"}><button className={"btn btn-glass-yellow"} style={{
                      flex: 1
                    }} onClick={() => {
                      const synth = {
                        name: currentDay.label || "Day",
                        icon: plan.icon || "\uD83D\uDCCB",
                        exercises: currentDay.exercises,
                        durationMin: currentDay.durationMin || null,
                        activeCal: currentDay.activeCal || null,
                        totalCal: currentDay.totalCal || null
                      };
                      openStatsPromptIfNeeded(synth, (woWithStats, _sr) => {
                        startPlanWorkout({
                          ...plan,
                          days: [{
                            ...currentDay,
                            durationMin: woWithStats.durationMin,
                            activeCal: woWithStats.activeCal,
                            totalCal: woWithStats.totalCal
                          }]
                        });
                      });
                    }}>{"✓ Complete Day"}</button></div><div style={{
                    display: "flex",
                    gap: S.s8,
                    marginTop: S.s8
                  }}><button className={`plan-sched-btn ${plan.scheduledDate ? "plan-sched-active" : ""}`} style={{
                      flex: 1,
                      padding: "8px 12px",
                      textAlign: "center"
                    }} onClick={() => openSchedulePlan(plan)}>{plan.scheduledDate ? "\uD83D\uDCC5 " + formatScheduledDate(plan.scheduledDate) : "\uD83D\uDCC5 Schedule"}</button>{plan.custom && <button className={"btn btn-danger btn-sm"} style={{
                      flex: 1
                    }} onClick={() => deletePlan(plan.id)}>{"🗑 Delete"}</button>}</div>{plan.custom && <button className={"btn btn-glass"} style={{
                    width: "100%",
                    marginTop: S.s8
                  }} onClick={() => startPlanWorkout(plan)}>{"📋 Mark Plan Complete"}</button>}</>;
              })()}</>;
          })()}{planView === "builder" && lazyMount(<PlanWizard editPlan={wizardEditPlan} templatePlan={wizardTemplatePlan} profile={profile} allExercises={allExercises} allExById={allExById} onSave={handlePlanWizardSave} onClose={() => {
            setPlanView("list");
          }} onCompleteDayStart={openStatsPromptIfNeeded} onStartPlanWorkout={startPlanWorkout} onDeletePlan={deletePlan} onSchedulePlan={openSchedulePlan} onOpenExEditor={openExEditor} showToast={showToast} />)}</> /* close plans tab React.createElement(React.Fragment) */ /* close activeTab==="plans" && () */
        ...
        /* ── CALENDAR TAB ────────────────────── */}{activeTab === "calendar" && (() => {
          ...
          return <><div className={"rpg-sec-header rpg-sec-header-center"} style={{
            ...
            /* Month navigator */<div className={"cal-nav"}><div className={"cal-nav-btn"} onClick={() => setCalViewDate(({
                y,
                m
              }) => m === 0 ? {
                y: y - 1,
            ...
            /* Day-of-week headers */<div className={"cal-grid"}>{dowNames.map(d => <div key={d} className={"cal-dow"}>{d}</div>)
            ...
            /* Legend */<div className={"cal-legend"}><div className={"cal-legend-item"}><div className={"cal-legend-dot"} style={{
                  background: "#b4ac9e"
            ...
            /* Selected day detail */{calSelDate && <div className={"cal-day-detail"}><div className={"cal-day-hdr"}><span>{selLabel}</span>{isSelToday && <span style={{
                  fontSize: FS.sm,
                  color: "#b4ac9e",
                  fontFamily: "'Inter',sans-serif"
                }}>{"Today"}</span>}</div>
              ...
              /* Scheduled items */{selSched.length > 0 && <><div style={{
                  ...
                  marginBottom: S.s6
                    ...
                    if (pl) {
                      initBuilderFromTemplate(pl, true);
                      setActiveTab("plans");
                    }
              ...
              /* Logged sessions — grouped by workout/plan */}{selLog.length > 0 && (() => {
                ...
                return <><div style={{
                  ...
                  /* Grouped workout/plan cards */{groupArr.map((entries, gi) => {
        ...
        /* ── LEADERBOARD TAB ─────────────────────── */}{activeTab === "leaderboard" && (() => {
          ...
          return <div> /* Header */
            <div className={"techniques-header"}><div className={"tech-hdr-left"}><div className={"tech-ornament-line tech-ornament-line-l"} /><span className={"tech-hdr-title"}>{"✦ Leaderboard ✦"}</span><div className={"tech-ornament-line tech-ornament-line-r"} /></div></div> /* Scope toggle: Friends / World */
            <div style={{
              display: "flex",
              gap: S.s4,
              marginBottom: S.s12,
            ...
            lbScope === "world" && <div style={{
              ...
              gap: S.s8,
              marginBottom: S.s10
            }}><MultiDrop label={"States"} icon={"\uD83D\uDCCD"} allLabel={"All States"} open={lbStateDropOpen} setOpen={setLbStateDropOpen} options={ALL_STATES} selected={lbStateFilters} setSelected={setLbStateFilters} /><MultiDrop label={"Countries"} icon={"\uD83C\uDF0D"} allLabel={"All Countries"} open={lbCountryDropOpen} setOpen={setLbCountryDropOpen} options={ALL_COUNTRIES} selected={lbCountryFilters} setSelected={setLbCountryFilters} /></div>} /* Category filter dropdown */
            <div style={{
              marginBottom: S.s12,
              position: "relative"
            ...
            myRow && <div style={{
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  }}>{myRank}</span></div> /* Name + class tag + subtitle */
                <div style={{
                  flex: 1,
                  minWidth: 0
                }}><div style={{
                    ...
                    flexWrap: "wrap"
                      ...
                      background: "rgba(180,172,158,.08)",
                      color: "#8a8478"
                    }}>{"\uD83C\uDF0D #" + lbWorldRanks[authUser.id]}</span>}</div></div> /* Stat value */
                <div style={{
                  textAlign: "right",
                  flexShrink: 0
                }}><div style={{
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      justifyContent: "center"
                        ...
                        color: rankColor,
                        fontFamily: "'Inter',sans-serif"
                      }}>{rank}</span></div> /* Name + class tag + subtitle */
                    <div style={{
                      flex: 1,
                      minWidth: 0
                    }}><div style={{
                        ...
                        flexWrap: "wrap"
                          ...
                          background: "rgba(180,172,158,.08)",
                          color: "#8a8478"
                        }}>{"\uD83C\uDF0D #" + worldRank}</span>}</div></div> /* Stat value */
                    <div style={{
                      textAlign: "right",
                      flexShrink: 0,
                      paddingLeft: 6
        ...
        /* ── QUESTS TAB ──────────────────────── */}{activeTab === "quests" && <><div className={"rpg-sec-header"}><div className={"rpg-sec-line rpg-sec-line-l"} /><span className={"rpg-sec-title"}>{"✦ Deeds & Quests ✦"}</span><div className={"rpg-sec-line rpg-sec-line-r"} /></div>
          /* Category filter */<div className={"quest-cats"}>{["All", "Cardio", "Strength", "Flexibility", "Consistency", "Competition"].map(cat => <div key={cat} className={`quest-cat-btn ${questCat === cat ? "on" : ""}`} onClick={() => setQuestCat(cat)}>{cat}</div>)}</div>

          /* Pending claims first */{QUESTS.filter(q => {
        ...
        /* ── HISTORY TAB ─────────────────────── */}{activeTab === "history" && (() => {
          ...
          return <div><div className={"rpg-sec-header"}><div className={"rpg-sec-line rpg-sec-line-l"} /><span className={"rpg-sec-title"}>{"✦ Guild Search ✦"}</span><div className={"rpg-sec-line rpg-sec-line-r"} /></div>{socialMsg && <div style={{
            ...
            /* Search result */{socialMsg === null && friendSearchResult && <div style={{
              background: "rgba(45,42,36,.18)",
              border: "1px solid rgba(180,172,158,.06)",
              borderRadius: R.r10,
              padding: "10px 12px",
        ...
        /* ── CHARACTER TAB ────────────────────── */}{activeTab === "character" && (() => {
          ...
          return <div style={{
            ...
            /* ══ AVATAR SUB-TAB ══════════════════════════ */{charSubTab === "avatar" && <div><div className={"char-section"} style={{
                textAlign: "center",
                padding: "52px 24px"
              }}><div style={{
                  fontSize: "2.6rem",
        ...
        /* ── PROFILE VIEW ─────────────────────── */}{activeTab === "profile" && !editMode && !securityMode && !notifMode && <div style={{
            ...
            return <div className={"profile-section"}><div className={"profile-rune-divider"} style={{
              ...
              myPublicId && <div style={{
                ...
                marginBottom: S.s6
                  ...
                  }}>{"Copy"}</span></span></div>} /* Display Name row */
              <ToggleRow label={"Display Name"} value={profile.playerName} rowKey={"displayName"} /> /* Divider */
              <div style={{
                height: 1,
                background: "rgba(180,172,158,.04)",
                margin: "0 0"
              }} /> /* Real Name row */
              <ToggleRow label={"First & Last Name"} value={realName || "Not set"} rowKey={"realName"} /> /* Legend */
              <div style={{
                display: "flex",
                gap: S.s10,
                justifyContent: "center",
          ...
          /* ── ABOUT YOU ── */{(profile.sportsBackground || []).length > 0 || profile.trainingStyle || profile.fitnessPriorities?.length > 0 || profile.disciplineTrait || profile.motto ? <div className={"profile-section"}><div className={"profile-rune-divider"} style={{
              margin: "0 0 10px"
        ...
        /* ── PROFILE EDIT ─────────────────────── */}{activeTab === "profile" && editMode && <><div style={{
            ...
            /* ── UNITS ── */<div><div className={"profile-rune-divider"} style={{
                margin: "0 0 10px"
            ...
            /* ── BODY STATS ── */<div><div className={"profile-rune-divider"} style={{
                margin: "0 0 10px"
            ...
            /* ── PREFERENCES ── */<div><div className={"profile-rune-divider"} style={{
                margin: "0 0 10px"
            ...
            /* ── ABOUT YOU ── */<div><div className={"profile-rune-divider"} style={{
                margin: "0 0 10px"
        ...
        /* ── SECURITY SETTINGS ─────────────────── */}{activeTab === "profile" && securityMode && <><div style={{
          ...
          /* ═══ Email Verification Status (with Show/Hide) ═══ */{authUser && <div style={{
            background: "rgba(45,42,36,.18)",
            border: "1px solid rgba(45,42,36,.2)",
            borderRadius: R.r10,
            padding: "10px 14px",
          ...
          /* ═══ CHANGE EMAIL — collapsible ═══ */<div className={"edit-panel"} style={{
            marginBottom: S.s12,
            padding: S.s0,
            overflow: "hidden"
          }}><div style={{
          ...
          /* ═══ MFA (TOTP) — collapsible ═══ */<div className={"edit-panel"} style={{
            marginBottom: S.s12,
            padding: S.s0,
            overflow: "hidden"
          }}><div style={{
              ...
              /* MFA IS enabled — show status, codes remaining, and disable option */}{mfaEnabled && !mfaRecoveryCodes && !mfaDisableConfirm && <div style={{
                ...
                /* Recovery codes remaining */<div style={{
                  background: "rgba(45,42,36,.15)",
                  border: "1px solid rgba(45,42,36,.2)",
                  borderRadius: R.lg,
                  padding: "10px 14px",
          ...
          /* ═══ Phone Number — collapsible ═══ */<div className={"edit-panel"} style={{
            marginBottom: S.s12,
            padding: S.s0,
            overflow: "hidden"
          }}><div style={{
          ...
          /* ═══ Set / Change Password — collapsible ═══ */<div className={"edit-panel"} style={{
            marginBottom: S.s12,
            padding: S.s0,
            overflow: "hidden"
          }}><div style={{
        ...
        /* ── NOTIFICATION PREFERENCES ─────────────────── */}{activeTab === "profile" && notifMode && <><div style={{
            ...
            fontStyle: "italic",
            textAlign: "center"
          }}>{"Changes save automatically. Email notifications require a verified email address."}</div></>}</div> /* scroll-area */</div>
      ...
      try {
        ...
        return <div className={"ex-editor-backdrop"} onClick={() => setExEditorOpen(false)}><div className={"ex-editor-sheet"} onClick={e => e.stopPropagation()} style={{
            "--mg-color": getMuscleColor(ed.muscleGroup || "chest")
              ...
              /* Category */<div className={"field"}><label>{"Category"}</label><div style={{
                  display: "flex",
                  gap: S.s6
              ...
              /* Muscle Group */<div className={"field"}><label>{"Muscle Group"}</label><div style={{
                  display: "flex",
                  gap: S.s4,
                  flexWrap: "wrap"
              ...
              /* Base XP */<div className={"field"}><label>{"Base XP per session "}<span style={{
                    fontSize: FS.sm,
                    color: "#8a8478",
                    fontStyle: "italic"
              ...
              /* ── Default Workout Values ───────────────── */<div className={"ex-editor-section"}><div className={"ex-editor-section-title"}>{"Default Values When Logging"}</div><div style={{
                ...
                /* Sets + Reps/Duration */<div className={"r2"}><div className={"field"}><label>{"Default Sets"}</label><input className={"inp"} type={"number"} min={"0"} max={"20"} value={ed.defaultSets != null ? ed.defaultSets : ""} placeholder={"0"} onChange={e => {
                      const v = e.target.value;
                      setEd({
                ...
                /* Weight — strength/endurance only */{hasWeightED && <><div className={"r2"}><div className={"field"}><label>{"Default Base Weight ("}{wUnit}{")"}</label><input className={"inp"} type={"number"} min={"0"} max={"2000"} step={metric ? "0.5" : "2.5"} value={ed.defaultWeightLbs ? metric ? lbsToKg(ed.defaultWeightLbs) : ed.defaultWeightLbs : ""} onChange={e => {
                        const v = e.target.value;
                        const lbs = v && metric ? kgToLbs(v) : v;
                        setEd({
              ...
              /* ── Exercise Details (optional) ─────── */<div className={"ex-editor-section-title"} style={{
                marginTop: S.s4
              }}>{"✦ Exercise Details (optional)"}</div>

              /* Muscles */<div className={"field"}><label>{"Target Muscles"}</label><input className={"inp"} value={ed.muscles || ""} onChange={e => setEd({
                  muscles: e.target.value
                })} placeholder={"e.g. Chest · Front Deltoids · Triceps"} /></div>

              /* Description */<div className={"field"}><label>{"Description"}</label><textarea className={"inp"} rows={3} value={ed.desc || ""} onChange={e => setEd({
                  desc: e.target.value
              ...
              /* Tips */<div className={"field"}><label>{"Form Tips (up to 3)"}</label>{[0, 1, 2].map(ti => <input key={ti} className={"inp"} style={{
                  marginBottom: S.s6
              ...
              /* ── Action Buttons ─────────────────── */<div className={"div"} /><div style={{
                display: "flex",
                gap: S.s8
              }}><button className={"btn btn-ghost btn-sm"} style={{
                  flex: 1
    ...
    /* ══ EXERCISE DETAIL MODAL ══════════════════ */}{detailEx && <div className={"modal-backdrop"} onClick={() => setDetailEx(null)}><div className={"modal-sheet"} onClick={e => e.stopPropagation()}><div className={"modal-img-row"}>{detailEx.images.map((src, i) => <img key={i} src={`${src}?w=420&h=260&fit=crop&q=80`} alt={detailEx.name} className={"modal-img"} onError={e => {
        ...
        /* Body */<div className={"modal-body"}><div style={{
            display: "flex",
            alignItems: "center",
            justifyContent: "space-between",
            marginBottom: S.s2
    ...
    /* ══ SAVE-TO-PLAN WIZARD ════════════════════ */}{savePlanWizard && <div className={"spw-backdrop"} onClick={() => setSavePlanWizard(null)} role={"dialog"} aria-modal={"true"} aria-label={"Save plan"}><div className={"spw-sheet"} onClick={e => e.stopPropagation()}><div className={"spw-hdr"}><div><div className={"spw-title"}>{"📋 Save To Plan"}</div><div style={{
          ...
          /* NEW PLAN fields */{spwMode === "new" && <><div className={"field"}><label>{"Plan Name"}</label><input className={"inp"} value={spwName} onChange={e => setSpwName(e.target.value)} placeholder={"Name your plan…"} /></div><div className={"field"}><label>{"Icon"}</label><div className={"icon-row"} style={{
                flexWrap: "wrap",
                gap: S.s6
    ...
    /* ══ SCHEDULE PICKER ════════════════════════ */}{schedulePicker && <div className={"sched-backdrop"} onClick={() => setSchedulePicker(null)}><div className={"sched-sheet"} onClick={e => e.stopPropagation()}><div style={{
        ...
        /* Target card */<div className={"sched-target"}><div className={"sched-target-icon"}>{schedulePicker.type === "plan" ? schedulePicker.plan.icon : schedulePicker.icon}</div><div><div className={"sched-target-name"}>{schedulePicker.type === "plan" ? schedulePicker.plan.name : schedulePicker.name}</div><div className={"sched-target-type"}>{schedulePicker.type === "plan" ? "Workout Plan" : "Exercise"}</div></div></div>

        /* Date picker */<div className={"field"}><label>{"Scheduled Date"}</label><input className={"inp"} type={"date"} min={todayStr()} value={spDate} onChange={e => setSpDate(e.target.value)} />{spDate && <div style={{
            fontSize: FS.fs65,
            color: "#b4ac9e",
            marginTop: S.s4
          }}>{(() => {
        ...
        /* If there's already a schedule, offer to clear it */{schedulePicker.type === "plan" && schedulePicker.plan.scheduledDate && <div style={{
          fontSize: FS.fs65,
          color: "#8a8478",
          fontStyle: "italic"
    ...
    /* ══ SAVE-AS-WORKOUT WIZARD ═════════════════ */}{saveWorkoutWizard && <div className={"saw-backdrop"} onClick={() => setSaveWorkoutWizard(null)}><div className={"saw-sheet"} onClick={e => e.stopPropagation()}><div className={"spw-hdr"}><div><div className={"spw-title"}>{"💪 Save As Workout"}</div><div style={{
          ...
          /* Workout name */<div className={"field"}><label>{"Workout Name"}</label><input className={"inp"} value={swwName} onChange={e => setSwwName(e.target.value)} placeholder={"Name your workout…"} /></div>
          /* Icon */<div className={"field"}><label>{"Icon"}</label><div className={"icon-row"} style={{
              flexWrap: "wrap",
              gap: S.s6
              ...
              marginBottom: S.s10
                ...
                <div style={{
                  ...
                  zIndex: 20
                    ...
                    }}>{PTYPE_LABELS[t]}</div>)}</div>}</div> /* Equipment */
                <div style={{
                  position: "relative",
                  flex: 1,
                  zIndex: 20
          ...
          setPendingSoloRemoveId(null);
            ...
            padding: "0 0 24px"
              ...
              padding: "0 14px"
                ...
                /* Secondary actions — add to existing workout / plan */<div style={{
                  display: "flex",
                  gap: S.s6
    ...
    /* ══ STATS PROMPT MODAL ══════════════════════ */}{statsPromptModal && createPortal(<div className={"modal-backdrop"} onClick={() => setStatsPromptModal(null)}><div className={"modal-sheet"} onClick={e => e.stopPropagation()} style={{
        ...
        padding: S.s0
          ...
          /* Make Reusable checkbox — only for one-off workouts */{statsPromptModal.wo.oneOff && <div className={"stats-prompt-reusable"} onClick={() => setSpMakeReusable(v => !v)}><div style={{
              width: 18,
              height: 18,
              borderRadius: R.r4,
    ...
    /* ══ CALENDAR EXERCISE READ-ONLY DETAIL MODAL ══ */}{calExDetailModal && <div className={"modal-backdrop"} onClick={() => setCalExDetailModal(null)}><div className={"modal-sheet"} onClick={e => e.stopPropagation()} style={{
        ...
        padding: S.s0
          ...
          /* Source info */{calExDetailModal.sourceName && <div style={{
            fontSize: FS.fs65,
            color: "#8a8478",
            fontStyle: "italic",
            padding: "6px 10px",
    ...
    /* ══ RETRO EDIT MODAL ═══════════════════════ */}{retroEditModal && (() => {
      ...
      return <div className={"modal-backdrop"} onClick={() => setRetroEditModal(null)}><div className={"modal-sheet"} onClick={e => e.stopPropagation()} style={{
          ...
          overflowY: "auto"
            ...
            /* Exercise list — editable */<div style={{
              marginBottom: S.s12
            }}>{rem.entries.map((e, i) => {
                const exData = allExById[e.exId];
                if (!exData) return null;
    ...
    /* ══ ADD TO EXISTING WORKOUT PICKER ════════ */}{addToWorkoutPicker && <div className={"modal-backdrop"} onClick={() => setAddToWorkoutPicker(null)}><div className={"modal-sheet"} onClick={e => e.stopPropagation()} style={{
        ...
        overflowY: "auto"
          ...
          /* Re-Usable Workouts */{(profile.workouts || []).filter(w => !w.oneOff).length > 0 && <><div style={{
              fontSize: FS.fs62,
              color: "#b4ac9e",
              textTransform: "uppercase",
              letterSpacing: ".08em",
      ...
      return <div className={"completion-backdrop"} onClick={() => {
        ...
        setScheduleWoDate("");
          ...
          /* Workout card */<div className={"completion-wo-card"}><span className={"completion-wo-icon"}>{wo.icon}</span><div><div className={"completion-wo-name"}>{wo.name}</div><div className={"completion-wo-sub"}>{wo.exercises.length}{" exercises · "}{formatXP(xp, {
                  prefix: "⚡ "
                })}</div></div></div>
          ...
          /* Options */<div style={{
            ...
            /* Option 2 — Completed on a past day */<div className={`completion-option ${inPickMode ? "sel" : ""}`} onClick={() => {
          ...
          /* XP preview — only for log actions */{(completionAction === "today" || inPickMode && pickerValue) && <div className={"completion-xp-preview"}><div className={"completion-xp-preview-label"}>{"XP to be claimed"}</div><div className={"completion-xp-preview-value"}>{"⚡ "}{xp.toLocaleString()}</div></div>}<div style={{
            display: "flex",
            gap: S.s8
          }}><button className={"btn btn-ghost btn-sm"} style={{
              flex: 1
    ...
    /* ══ LOG ENTRY EDIT MODAL ════════════════════ */}{logEditModal && logEditDraft && (() => {
      ...
      return <div className={"ledit-backdrop"} onClick={() => setLogEditModal(null)}><div className={"ledit-sheet"} onClick={e => e.stopPropagation()}><div style={{
          ...
          /* Source info */{(d.sourcePlanName || d.sourceWorkoutName) && <div style={{
            fontSize: FS.fs65,
            color: "#8a8478",
            fontStyle: "italic",
            padding: "6px 10px",
          ...
          /* Sets + Reps/Duration */<div className={"r2"}><div className={"field"}><label>{"Sets"}</label><input className={"inp"} type={"number"} min={"1"} max={"99"} value={d.sets || 1} onChange={e => setD({
          ...
          /* Weight */{showWeight && <div className={"field"}><label>{"Weight ("}{wUnit}{")"}</label><input className={"inp"} type={"number"} min={"0"} step={"2.5"} value={d.weightLbs ? metric ? lbsToKg(d.weightLbs) : d.weightLbs : ""} placeholder={"0"} onChange={e => {
    ...
    /* ══ MAP OVERLAY ═════════════════════════════ */}{mapOpen && (() => {
      ...
      return <div style={{
        ...
        /* Zoom controls + map */<MapSVG myPos={myPos} myRegion={myRegion} friendPositions={friendPositions} mapTooltip={mapTooltip} setMapTooltip={setMapTooltip} travelActive={travelActive} profile={profile} />

        /* Tooltip / travel panel */{mapTooltip && <div style={{
          width: "100%",
          maxWidth: 420,
          marginTop: S.s10,
        ...
        /* Active travel banner */{travelActive && <div style={{
          width: "100%",
          maxWidth: 420,
          marginTop: S.s10,
          padding: "10px 14px",
