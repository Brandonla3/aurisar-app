...
import { UI_COLORS, NO_SETS_EX_IDS } from '../../data/constants';
import { getMuscleColor, getTypeColor } from '../../utils/xp';
import { isMetric, weightLabel, distLabel } from '../../utils/units';
 ...
 * (PR #116).
 *
 * Two-pane UI: exercise search/filter list (pickerConfigOpen=false) and
 * per-exercise configuration form (pickerConfigOpen=true).
...
const WorkoutExercisePicker = memo(function WorkoutExercisePicker({
  ...
  pickerConfigOpen, setPickerConfigOpen,
  // Exercise data
  allExercises,
  allExById,
  // Profile (for units)
        ...
        {!pickerConfigOpen ? (
          <>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: S.s10 }}>
              ...
              <div style={{ display: "flex", gap: S.s6 }}>
                {pickerSelected.length > 0 && (
                  <button className={"btn btn-gold btn-xs"} onClick={() => setPickerConfigOpen(true)}>{"Configure & Add →"}</button>
                )}
          ...
          <>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: S.s10 }}>
              ...
              <div className={"sec"} style={{ margin: 0, border: "none", padding: S.s0 }}>
                {"Configure "}{pickerSelected.length}{" Exercise"}{pickerSelected.length !== 1 ? "s" : ""}
              </div>
              <button className={"btn btn-gold btn-sm"} onClick={commitPickerToWorkout}>{"Add to Workout ✓"}</button>
            </div>
                ...
                <div key={entry.exId} style={{ background: "rgba(45,42,36,.12)", border: "1px solid rgba(180,172,158,.05)", borderRadius: R.r10, padding: "10px 12px", marginBottom: S.s8 }}>
                    ...
                    {isCardio ? (
                      <>
                        <div className={"field"} style={{ flex: 1.6, minWidth: 70, marginBottom: S.s0 }}>
                          ...
                          <input className={"inp"} style={{ padding: "6px 8px" }} type={"text"} inputMode={"numeric"} value={entry._durHHMM || ""} onChange={e => pickerUpdateEx(entry.exId, "_durHHMM", e.target.value)} onBlur={e => {
                        ...
                        <div className={"field"} style={{ flex: 0.8, minWidth: 50, marginBottom: S.s0 }}>
                          <label>{"Seconds"}</label>
                          <input className={"inp"} style={{ padding: "6px 8px", textAlign: "center" }} type={"number"} min={"0"} max={"59"} value={entry._durSec || ""} onChange={e => {
                  ...
                  {(entry.extraRows || []).map((row, ri) => (
                    <div key={ri} style={{ display: "flex", gap: S.s4, marginBottom: S.s4, padding: "6px 8px", background: "rgba(45,42,36,.18)", borderRadius: R.r5, alignItems: "center", flexWrap: "wrap" }}>
                      ...
                      {isCardio ? (
                        <>
                          <input className={"inp"} style={{ flex: 1.5, minWidth: 50, padding: "4px 8px", fontSize: FS.lg }} type={"text"} inputMode={"numeric"} placeholder={"HH:MM"} value={row.hhmm || ""} onChange={e => { const rr = [...(entry.extraRows || [])]; rr[ri] = { ...rr[ri], hhmm: e.target.value }; pickerUpdateEx(entry.exId, "extraRows", rr); }} onBlur={e => { const rr = [...(entry.extraRows || [])]; rr[ri] = { ...rr[ri], hhmm: normalizeHHMM(e.target.value) }; pickerUpdateEx(entry.exId, "extraRows", rr); }} />
                          <input className={"inp"} style={{ flex: 0.7, minWidth: 36, padding: "4px 8px", fontSize: FS.lg }} type={"number"} min={"0"} max={"59"} placeholder={"Sec"} value={row.sec || ""} onChange={e => { const rr = [...(entry.extraRows || [])]; rr[ri] = { ...rr[ri], sec: e.target.value }; pickerUpdateEx(entry.exId, "extraRows", rr); }} />
                          <input className={"inp"} style={{ flex: 1, minWidth: 40, padding: "4px 8px", fontSize: FS.lg }} type={"text"} inputMode={"decimal"} placeholder={dUnit} value={row.distanceMi || ""} onChange={e => { const rr = [...(entry.extraRows || [])]; rr[ri] = { ...rr[ri], distanceMi: e.target.value }; pickerUpdateEx(entry.exId, "extraRows", rr); }} />
                          {isTreadEx && <input className={"inp"} style={{ flex: 0.7, minWidth: 34, padding: "4px 8px", fontSize: FS.lg }} type={"number"} min={"0.5"} max={"15"} step={"0.5"} placeholder={"Inc"} value={row.incline || ""} onChange={e => { const rr = [...(entry.extraRows || [])]; rr[ri] = { ...rr[ri], incline: e.target.value }; pickerUpdateEx(entry.exId, "extraRows", rr); }} />}
                          {isTreadEx && <input className={"inp"} style={{ flex: 0.7, minWidth: 34, padding: "4px 8px", fontSize: FS.lg }} type={"number"} min={"0.5"} max={"15"} step={"0.5"} placeholder={"Spd"} value={row.speed || ""} onChange={e => { const rr = [...(entry.extraRows || [])]; rr[ri] = { ...rr[ri], speed: e.target.value }; pickerUpdateEx(entry.exId, "extraRows", rr); }} />}
                        </>
                      ) : (
                        <>
                          {!noSets && <input className={"inp"} style={{ flex: 1, minWidth: 40, padding: "4px 8px", fontSize: FS.lg }} type={"text"} inputMode={"decimal"} placeholder={"Sets"} value={row.sets || ""} onChange={e => { const rr = [...(entry.extraRows || [])]; rr[ri] = { ...rr[ri], sets: e.target.value }; pickerUpdateEx(entry.exId, "extraRows", rr); }} />}
                          <input className={"inp"} style={{ flex: 1, minWidth: 40, padding: "4px 8px", fontSize: FS.lg }} type={"text"} inputMode={"decimal"} placeholder={"Reps"} value={row.reps || ""} onChange={e => { const rr = [...(entry.extraRows || [])]; rr[ri] = { ...rr[ri], reps: e.target.value }; pickerUpdateEx(entry.exId, "extraRows", rr); }} />
                          <input className={"inp"} style={{ flex: 1, minWidth: 40, padding: "4px 8px", fontSize: FS.lg }} type={"text"} inputMode={"decimal"} placeholder={wUnit} value={row.weightLbs || ""} onChange={e => { const rr = [...(entry.extraRows || [])]; rr[ri] = { ...rr[ri], weightLbs: e.target.value }; pickerUpdateEx(entry.exId, "extraRows", rr); }} />
                        </>
                      )}
                  ...
                  <button className={"btn btn-ghost btn-xs"} style={{ width: "100%", marginTop: S.s4, fontSize: FS.sm, color: "#8a8478", borderStyle: "dashed" }} onClick={() => {
                    const rr = [...(entry.extraRows || []), isCardio ? { hhmm: "", sec: "", distanceMi: "", incline: "", speed: "" } : { sets: "", reps: "", weightLbs: "" }];
                    pickerUpdateEx(entry.exId, "extraRows", rr);
                  }}>{"＋ Add Row (e.g. "}{isCardio ? "interval" : "progressive set"}{")"}</button>
                </div>
              );
            })}
          </>
