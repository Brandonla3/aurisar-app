...
/**
 * Guild (Social) tab â€” extracted from the inline IIFE in App.jsx as part of
...
// Hoist out of render â€” same 100-level table on every call
...
const GuildTab = memo(function GuildTab({
  ...
  return <div><div className={"rpg-sec-header"}><div className={"rpg-sec-line rpg-sec-line-l"} /><span className={"rpg-sec-title"}>{"âœ¦ Guild Search âœ¦"}</span><div className={"rpg-sec-line rpg-sec-line-r"} /></div>{socialMsg && <div style={{
      fontSize: FS.fs75,
      color: socialMsg.ok === true ? UI_COLORS.success : socialMsg.ok === false ? UI_COLORS.danger : "#b4ac9e",
      marginBottom: S.s10,
      padding: "8px 12px",
      background: socialMsg.ok === true ? "rgba(46,204,113,.06)" : socialMsg.ok === false ? "rgba(231,76,60,.06)" : "rgba(74,69,59,.16)",
      border: `1px solid ${socialMsg.ok === true ? "rgba(46,204,113,.2)" : socialMsg.ok === false ? "rgba(231,76,60,.2)" : "rgba(74,69,59,.3)"}`,
      borderRadius: R.lg,
      textAlign: "center"
      ...
      marginBottom: S.s8
        ...
        padding: "8px 12px",
        fontSize: FS.fs82
      }} placeholder={"Email or Account ID (#A7XK9M)â€¦"} value={friendSearch} onChange={e => {
      ...
      }} disabled={friendSearchLoading || !friendSearch.trim()} onClick={searchFriendByEmail}>{friendSearchLoading ? "â€¦" : "Search"}</button></div>
      {
        /* Search result */
        ...
        marginBottom: S.s12
          ...
          return <div><div style={{
              ...
              alignItems: "center",
              gap: S.s10
            }}><div className={"friend-avatar"}>{uCls?.icon || "âš”ï¸"}</div><div style={{
                flex: 1
                    ...
                    fontWeight: 400,
                    marginLeft: S.s6
                  }}>{"#" + u.publicId}</span>}</div><div className={"friend-meta"}>{uCls?.name || "Unknown"}{friendSearchResult.matchType === "account_id" ? " Â· Found by Account ID" : " Â· Found by email"}</div></div>{!ex && <button className={"btn btn-gold btn-xs"} onClick={() => sendFriendRequest(u.id)}>{"+ Add"}</button>}{ex?.status === "pending" && <div style={{
                display: "flex",
                ...
                gap: S.s4
                  ...
                  color: "#9a9488",
                  fontStyle: "italic"
                }}>{"Request pendingâ€¦"}</span><button className={"btn btn-ghost btn-xs"} style={{
                  fontSize: FS.fs58,
              ...
              }}>{"Already friends âœ“"}</span>}</div></div>;
        })() : <div style={{
          fontSize: FS.fs75,
          color: "#9a9488",
          fontStyle: "italic"
      ...
      /* Incoming requests */}{friendRequests.length > 0 && <><div className={"sec"} style={{
          marginBottom: S.s8
        }}>{"âš”ï¸ Incoming Requests"}</div>{friendRequests.map(r => <div key={r.reqId} className={"req-card"}><div style={{
            flex: 1
          }}><div style={{
              fontSize: FS.fs78,
              color: "#d4cec4"
      ...
      /* Incoming shared items */}{incomingShares.length > 0 && <><div className={"sec"} style={{
          marginBottom: S.s8
        }}>{"ðŸ“¦ Incoming Shares"}</div>{incomingShares.map(s => <div key={s.id} className={"req-card"} style={{
          flexDirection: "column",
          alignItems: "stretch",
          gap: S.s8
        }}><div style={{
            ...
            gap: S.s8
              ...
              fontSize: "1.1rem"
            }}>{s.type === "workout" ? "ðŸ’ª" : "âš¡"}</span><div style={{
              flex: 1
            }}><div style={{
                fontSize: FS.fs78,
                color: "#d4cec4"
            ...
            fontStyle: "italic",
            paddingLeft: 28
          }}>{s.parsedItem.desc.slice(0, 80)}{s.parsedItem.desc.length > 80 ? "â€¦" : ""}</div>}<div style={{
            display: "flex",
            gap: S.s6,
            paddingLeft: 28
          }}><button className={"btn btn-gold btn-xs"} style={{
              flex: 1
            }} onClick={() => acceptShare(s)}>{"âœ“ Add to Mine"}</button><button className={"btn btn-ghost btn-xs"} style={{
              flex: 1
      ...
      /* Outgoing pending requests */}{outgoingRequests.length > 0 && <><div className={"sec"} style={{
          marginBottom: S.s8,
          marginTop: S.s12
        }}>{"ðŸ“¤ Pending Sent ("}{outgoingRequests.length}{")"}</div>{outgoingRequests.map(r => <div key={r.reqId} className={"req-card"}><div style={{
            flex: 1
          }}><div style={{
              fontSize: FS.fs78,
              color: "#d4cec4"
              ...
              color: "#9a9488",
              marginTop: S.s2
            }}>{"Awaiting their responseâ€¦"}</div></div><button className={"btn btn-ghost btn-xs"} style={{
            flexShrink: 0,
            fontSize: FS.fs65,
            color: UI_COLORS.danger,
            borderColor: "rgba(231,76,60,.3)"
      ...
      /* Friends list */}<div style={{
        ...
        marginTop: friendRequests.length > 0 || incomingShares.length > 0 || outgoingRequests.length > 0 ? 12 : 0
          ...
          border: "none",
          padding: S.s0
        }}>{"ðŸ‘¥ My Party ("}{friends.length}{")"}</div>{authUser && <button className={"btn btn-ghost btn-xs"} style={{
          fontSize: FS.fs58
        }} onClick={() => {
          loadSocialData();
          loadIncomingShares();
        }}>{socialLoading ? "â€¦" : "â†º Refresh"}</button>}</div>{!authUser && <div className={"empty"}>{"Sign in to see your friends."}</div>}{authUser && socialLoading && <div className={"empty"}>{"Loading your partyâ€¦"}</div>}{authUser && !socialLoading && friends.length === 0 && <div className={"empty"}>{"No friends yet."}<br />{"Search by email to find other warriors."}</div>}{friends.map(f => {
        const fCls = f.chosenClass ? CLASSES[f.chosenClass] : null;
        const fLevel = levelFor(f.xp || 0);
        const recentEv = friendRecentEvents[f.id];
        const recent = recentEv ? `${recentEv.exercise_icon || "ðŸ’ª"} ${recentEv.exercise_name || recentEv.exercise_id}` : null;
        return <div key={f.id} className={"friend-card"}><div className={"friend-card-top"}><div className={"friend-avatar"} style={{
              borderColor: fCls?.color || "rgba(74,69,59,.3)"
            }}>{fCls?.icon || "âš”ï¸"}</div><div style={{
              flex: 1,
              minWidth: 0
            }}><div style={{
                display: "flex",
                ...
                justifyContent: "space-between"
                  ...
                  gap: S.s4
                    ...
                    fontSize: FS.fs55,
                    padding: "2px 8px"
                  }} onClick={() => openDmWithUser(f.id)}>{"ðŸ’¬ Chat"}</button><button className={"btn btn-ghost btn-xs"} style={{
                    fontSize: FS.fs55,
                    color: "#b4ac9e",
                    padding: "2px 6px"
